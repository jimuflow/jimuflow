...
from jimuflow.components.web_automation import HoverWebElementComponent
...
async def test_execute(http_server, headless):
    async with create_component_context(HoverWebElementComponent) as component:
        ...
        component.node.inputs = {
            "webPage": 'wp',
            "elementUri": add_test_web_element(component, '//*[@id="test6"]'),
        }
        assert (await component.execute()) == ControlFlow.NEXT
        assert (await page.locator('//*[@id="test6"]').text_content()) == '鼠标已悬停'
